...
void inicializa_coeficientes(void) {	
	for (int i=0; i<=(M_NUMBER); i++) {
		...
		{
			a[i]=sin(2*M_PI*F_CORTE*(i-M_NUMBER/2)/(F_AMOSTRAGEM))/(M_PI*(i-M_NUMBER/2));
		}
	}
...
int main (){
	...
	for (int i =0; i<301; i++)
	{
		sinal[i]= (int)1023/2+1024/2*sin(2*M_PI*23*i/F_AMOSTRAGEM);
	}
	...
	for (int i =0; i<301; i++)
	{
		printf("%d,",(int)sinal_filtrado[i]/4);
	}
	printf("\n");
	printf("\n");
